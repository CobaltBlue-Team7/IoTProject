var mysql = require("mysql");var clsMysql = function(host, port, user, password, database){        this.host = host;        this.port = port;        this.user= user;        this.password = password;        this.database = database;                this.connection = null;};clsMysql.prototype = {                connect: function(){                        this.connection = mysql.createConnection({                                host: this.host,                                port: this.port,                                user: this.user,                                password: this.password,                                database: this.database                        });                },                close: function(){                        if(this.connection){                                this.connection.end();                                this.connection = null;                        }                },                select: function(query, callback){                        console.log(query);                        this.connection.query(query, callback);                }};    var objMysql = new clsMysql("127.0.0.1", 3306, "root", "", "mysql");    objMysql.connect();        objMysql.connection.query("SELECT * FROM user", function(err, rows){        console.log(err);        console.log(rows.length);    });    objMysql.close(); #include <ESP8266WiFi.h>
